...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($tipoDoc == "Justificantes") {
        ...
        // Verificar si ya se ha alcanzado el límite de solicitudes
        $query =
            "SELECT cantidad FROM conteo WHERE Id_alumno = ? AND Tipo_documento = ? AND Fecha_conteo BETWEEN ? AND ?";
        ...
        $cantidadSolicitudes = $row["cantidad"] ?? 0;
        $stmt->close();
            ...
            if ($selectSql) {
                if (mysqli_num_rows($selectSql) > 0) {
                    ...
                    $updateSql = $con->query($updateQuery);
                     $conteoQuery =
                        "INSERT INTO conteo (Id_alumno, Tipo_documento, Fecha_conteo, cantidad) VALUES (?, ?, ?, 1) ON DUPLICATE KEY UPDATE cantidad = cantidad + 1";
                    ...
                    $insertQuery =
                        "INSERT INTO Solicitudes_alumno (Id_alumno, Tipo_documento, fecha_solicitud) VALUES (?, ?, ?)";
